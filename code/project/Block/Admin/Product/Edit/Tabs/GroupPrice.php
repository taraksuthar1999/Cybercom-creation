   ...
   // protected $product = null;
    ...
    public function setCustomerGroups($customerGroups = null)
    {
        if (!$customerGroups) {
            ...
            $query = "SELECT cg.`id` as groupId,cg.`name` as groupName,if(p.`price` IS NULL,'{$this->getTableRow()->price}',p.`price`) as price,cgp.`entityId`,cgp.`price` as groupPrice FROM `customergroup` AS cg LEFT JOIN `product_customer_group_price` AS cgp ON cg.`id`=cgp.`customerGroupId` AND cgp.`productId`='{$this->getTableRow()->id}' LEFT JOIN `product` as p ON cgp.`productId`= p.`id`";
